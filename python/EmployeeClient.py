...
def run():
    with grpc.insecure_channel(const.IP + ':' + const.PORT) as channel:
        ...
        print('Employee data: ' + str(response))
        ...
        print('Updated title: ' + response.status)
        ...
        # Novos endpoints
        response = stub.SearchByName(EmployeeService_pb2.EmployeeNameSearch(name='Saravanan'))
        print('SearchByName: ' + str(response))

        response = stub.CountEmployees(EmployeeService_pb2.EmptyMessage())
        print('CountEmployees: ' + str(response.count))

        response = stub.UpdateEmployeeName(EmployeeService_pb2.EmployeeNameUpdate(id=101, name='Saravanan Silva'))
        print('UpdateEmployeeName: ' + response.status)
